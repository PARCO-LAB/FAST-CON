...
__device__ __forceinline__ void KVisit(	const int dest, dist_t* colors,
										int* Queue, int& founds, const int level, volatile long long int* HashTable, const int src) {
	#if ATOMICCAS
	...
	// Found a node that already has a color -> Connection found!
...
__device__ __forceinline__ void EdgeVisit(	 	   int* __restrict__	devEdge,
												dist_t* __restrict__	colors,
												   ...
												   int* __restrict__	devF2SizePrt,
													int start, int end,
													int* Queue, int& founds, const int level, volatile long long int* HashTable, const int index) {
#if SAFE == 0
	for (int k = start + (Tid & _Mod2<WARP_SZ>::VALUE); k < end; k += WARP_SZ) {
		const int dest = devEdge[k];

		KVisit<BlockDim, DUP_REM>(dest, colors, Queue, founds, level, HashTable, index);
	}
#elif SAFE == 1
	bool flag = true;
