...
__device__ __forceinline__ void BFS_BlockKernelB (	      int* __restrict__		devNode,
		...
		for (int t = Tid >> _Log2<WARP_SZ>::VALUE; t < FrontierSize; t += BlockDim / WARP_SZ) {
			...
			EdgeVisit<BlockDim, WARP_SZ, DUP_REM * 2>(devEdge, colors, NULL, NULL, index, start, end, Queue, founds, level, HashTable);
		}
		...
		for (int t = 0; t < DIV(BLOCK_FRONTIER_LIMIT, BlockDim); t++) {
			...
			if (indexT < FrontierSize) {
				...
				EdgeVisit<BlockDim, WARP_SZ, DUP_REM>(devEdge, colors, devF2, F2SizePtr, index, start, end, Queue, founds, level, HashTable);
			}
		}*/
...
#define fun(a)		BFS_BlockKernelB<1024, (a), DUP_REM>\
							(devNodes, devEdges, colors, SMemF1, SMemF2, devF2, F2SizePtr, FrontierSize, level, HashTable);
