...
int main(int argc, char* argv[]) {
	...
	srand(time(NULL));
	int s = mt::randf_co() * graph.V, t = mt::randf_co() * graph.V;
	graph.BfsInit(s, hostDistance);
	
	std::cout << "Computing Seq. ST-CON...\n" << "Source: " << s << " Target: " << t << "\n" <<std::flush;
	Timer<HOST> TM;
	TM.start();

	graph.bfs(t);

	TM.getTime("Host ST-CON");

	cudaGraph devGraph(graph);

	devGraph.cudaFASTCON_N(N_OF_TESTS);			// 		<-- FAST-CON
}


void Parameters(int argc, char* argv[], GDirection &GDir, int& nof_tests) {
	std::string errString(
	"Syntax Error:\n\n fast-con-msmf <graph_path> [ <graph_direction> ] [ -n <number_of_tests>]\n\n\
        <graph_direction>:\n\
