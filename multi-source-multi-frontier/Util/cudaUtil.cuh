...
#define cudaError(msg)		cudaDeviceSynchronize(); cudaUtil::__getLastCudaError (msg, __FILE__, __LINE__)
...
namespace cudaUtil {
	...
	template<class T>
	__global__ void scatterKernel(int* toScatter, const int nof_items, T* Dest);
	...
	inline void cudaStatics() {
		...
		if (devProp.multiProcessorCount * devProp.maxThreadsPerMultiProcessor != MAX_CONCURR_TH)
			error("Wrong MAX_CONCURR_TH configuration: " << devProp.multiProcessorCount * devProp.maxThreadsPerMultiProcessor <<
					" vs. " << MAX_CONCURR_TH)
		#endif
	}
	...
	__global__ void scatterKernel(int* toScatter, const int nof_items, T* Dest) {
		...
		// if (ID < nof_items)
		// 	Dest[ toScatter[ID] ] = ID;

		if (ID == 0) {
			for (int count = 0; count < nof_items; ++count) {
				Dest[toScatter[count]] = count;
			}

			for (int count = 0; count < (nof_items / SOURCES_PER_BLOCK); ++count) {
				for (int source = 0; source < SOURCES_PER_BLOCK; ++source) {
					toScatter[SHARED_MEMORY * count + source] = toScatter[SOURCES_PER_BLOCK * count + source];
					if (count > 0)
						toScatter[SOURCES_PER_BLOCK * count + source] = 0;

					// printf("block %d, tid %d, toscatter[SHARED_MEMORY] %d, color %d\n",count, source, toScatter[SHARED_MEMORY * count + source], Dest[toScatter[SHARED_MEMORY * count + source]]);
				}
			}
		}
	}
